 ...
 * attached**. Checking format alone, the value could be `http://169.254.169.254/` or an internal
 * service address, turning the registry lookup path into a channel for reading the internal network.
 *
 ...
 *
 * Redirects are not followed (`source-adapter.ts`). Following them would change the address
 ...
 *
 * Only `https` is accepted. Credentials are attached, so a plaintext request exposes them
...
export function assertEndpointShape(endpoint: string): URL {
  ...
  if (url.protocol !== "https:") {
    throw new EndpointNotAllowedError("Source endpoint must be https");
  }
  ...
  if (BLOCKED_HOSTNAMES.has(host) || BLOCKED_SUFFIXES.some((suffix) => host.endsWith(suffix))) {
    throw new EndpointNotAllowedError(`Internal hosts cannot be sources: ${host}`);
  }

  if (isIP(host) !== 0 && isPrivateAddress(host)) {
    throw new EndpointNotAllowedError(`Private or reserved addresses cannot be sources: ${host}`);
  }
 ...
 * connected addresses differ. Callers connect only to the addresses returned here (`source-fetch.ts`).
 */
