 ...
 *
 * The checksum pins what a migration *does*. Comments do not change that, so rewording them
 * must not stop a database that already applied the file. Code and string literals still
 * count: changing either is a different migration.
 */
...
describe("migrationChecksum", () => {
  ...
  it("does not change when only comments change", () => {
    const reworded = "-- reworded comment\nCREATE TABLE t (id INT); -- end\n";
    expect(migrationChecksum(reworded)).toBe(migrationChecksum(original));
  });
...
describe("judgeChecksum", () => {
  ...
  it("upgrades a pinned earlier revision to the pinned current revision", () => {
    const earlier = "CREATE TABLE t (id INT); SELECT 'old text';";
    const revised = "CREATE TABLE t (id INT); SELECT 'new text';";
    const pinned = { full: [], earlier: [migrationChecksum(earlier)], normalized: migrationChecksum(revised) };
    expect(judgeChecksum({ recorded: migrationChecksum(earlier), sql: revised, legacy: pinned })).toBe(
      "upgrade",
    );
    const other = "CREATE TABLE t (id INT); SELECT 'other text';";
    expect(judgeChecksum({ recorded: migrationChecksum(earlier), sql: other, legacy: pinned })).toBe(
      "mismatch",
    );
  });
});
...
describe("existing migrations", () => {
  it("match their pinned checksums", () => {
    // Every file listed in LEGACY_CHECKSUMS must normalize to its pinned revision. A mismatch
    // means a migration was edited in place without regenerating and reviewing the pin.
